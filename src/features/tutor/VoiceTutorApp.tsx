...
function TutorWorkspace() {
  ...
  const hasStartedConversation =
    activeSession?.messages.some((message) => message.role === "student") ||
    conversation.status === "connecting" ||
    conversation.status === "connected";
  ...
  return (
    <main
      className={`workspace workspace--${studioBackdrop} ${isSidebarCollapsed ? "is-sidebar-collapsed" : ""}`}
      data-theme-status={studioTheme}
    >
      <AppSidebar
      ...
      />
      <section className="chat" aria-label="Teach Me chat">
        <header className="chat__topbar">
          <div className="chat__actions">
            <button
              type="button"
              className="chat__pack-button btn-stop-motion"
              onClick={handleOpenStudyPack}
              disabled={!canShowStudyPack || isStudyPackPending}
              title={canShowStudyPack ? "Open study pack" : "Study pack appears after a short conversation"}
              data-wiggle
              data-sprite
            >
              <span className="btn-stop-motion__inner">
                <span className="btn-stop-motion__back" aria-hidden="true">
                  <svg className="btn-stop-motion__back-svg" xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 678 82" fill="none" preserveAspectRatio="none">
                ...
                <span className="btn-stop-motion__icon" aria-hidden="true">
                  <span className="before__100" />
                  <svg className="btn-stop-motion__icon-svg" xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 160 40" fill="none">
                ...
                <span data-wiggle-target className="btn-stop-motion__p">
                  {isStudyPackPending
                    ? "Building pack"
                    : activeSession?.studyNote
                      ? "Study pack ready"
                      : canShowStudyPack
                        ? "Build study pack"
                        : "Study pack"}
            ...
            {shouldShowHeaderStatus && (
              <div className="chat__status">
                <GlassEffect />
                <span className={conversation.status === "connected" ? "is-live" : ""} />
                {statusMessage}
              </div>
            )}
          </div>
            ...
            <div className="chat__composer-toolbar">
              <div className="chat__composer-actions">
                ...
                <button
                  ...
                  title={composerVoiceLabel}
                >
                  {isVoiceActive ? <XIcon size={20} /> : <Mic size={20} />}
                </button>
              </div>
            </div>
          </div>
        ...
        {isStudyPackOpen && (
          <StudyPackDrawer
            note={activeSession?.studyNote ?? null}
            isPending={isStudyPackPending}
            activeTab={activePackTab}
            onTabChange={setActivePackTab}

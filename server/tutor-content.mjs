...
export function localTutorReply(input, personality = "athena") {
  ...
  const asksForPractice = normalized.includes("quiz") || normalized.includes("test me") || normalized.includes("question me");
  const asksForModel =
  ...
  if (isGreeting) {
    return `${opener} Ready when you are. What are we working on?`;
  }
  ...
  if (!isChemistry) {
    ...
    if (asksForPractice) {
      ...
      if (genericTopic !== "Study Session") {
        return `${opener} ${genericPracticePromptForTitle(genericTopic)}`;
      }

      return `${opener} What subject or topic should I test you on?`;
    }
    ...
    if (subject) {
      return `${opener} I’ve got ${subject.label}. What level are we aiming at?`;
    }

    if (genericTopic !== "Study Session") {
      return `${opener} I’ve got ${genericTopic.toLowerCase()}. What part feels most important to practise?`;
    }

    return `${opener} What subject or topic should we work on?`;
  }

  if (asksForPractice) {
    return "One check: what phrase means the minimum energy particles need for a successful collision?";
  }
  if (asksForModel) {
...
function genericPracticePromptForTitle(topicTitle) {
  switch (topicTitle) {
    case "Biology Practice":
      return "One biology check: in photosynthesis, what two raw materials does the plant need to make glucose?";
    case "Maths Practice":
      return "One maths check: what is the first step you would take to solve this type of problem?";
    case "History Practice":
      return "One history check: can you name one cause, one event, and one consequence for this topic?";
    case "Physics Practice":
      return "One physics check: what key equation, force, or energy transfer is involved here?";
    case "English Practice":
      return "One English check: what quote or technique would you use as your evidence?";
    default:
      return `One check on ${topicTitle.toLowerCase()}: what is the key idea in one sentence?`;
  }
}
